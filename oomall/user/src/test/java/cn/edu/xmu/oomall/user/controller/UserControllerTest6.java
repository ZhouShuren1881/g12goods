...
public class UserControllerTest6 {
     ...
     */
    @Test
    public void putUsersIdBan1() throws Exception {

        String responseString = null;
        String token = this.createTestToken(9L, 2L, 10000);

        try {
            responseString = this.mvc.perform(put("/other/users/1/ban").header("authorization", token))
     ...
     */
    @Test
    public void putUsersIdBan2() throws Exception {

        String responseString = null;
        String token = this.createTestToken(9L, 2L, 10000);

        try {
            responseString = this.mvc.perform(put("/other/users/6/ban").header("authorization", token))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType("application/json;charset=UTF-8"))
                    .andReturn().getResponse().getContentAsString();
        }catch (Exception e){
